...
- In scope: New `.claude-plugin` + `skills/` tree inside this repo; slim `CLAUDE.md`; templates relocated as plugin-internal canon; `CONTEXT.md` adoption; a pilot consumer repo.
- Out of scope (this DEC): retrofitting other consumer repos, advanced hooks, settings.json automation.
...
- Expected evidence: plugin.json validates, all 7 SKILL.md files have correct frontmatter, slim CLAUDE.md ≤ 30 lines, pilot consumer repo exercises each skill on real work without regressions.
...
2. **Replace scaffold with Matt's skills wholesale** — adopt Matt's plugin and discard our continuity layer. Rejected: throws away the continuity layer (markers, session-log, decision records) which is genuinely useful.
...
1. **Migration scope**: refactor `engineering-scaffold-template`; pilot the new model in a consumer repo; defer other consumer repos until the pilot proves stable.
2. **Distribution**: hybrid — plugin core + per-repo override drop-ins.
3. **Relationship to Matt's skills**: adopt wholesale. Build only the continuity and engineering-discipline layer that Matt doesn't have.
...
5. **Skill inventory (7 skills)**: `/resume`, `/decide`, `/handoff`, `/threat-model`, `/marker`, `/verify`, `/release-ready`.
...
- **Risk**: skill drift between plugin core and a consumer repo's local overrides (hybrid distribution can get messy).
...
The pre-migration `CLAUDE.md` and structure are preserved in git history at the parent commit of this DEC. Rollback is `git revert` of the migration commits. No data loss possible — work-index, session-log, and existing decisions/experiments are unchanged in this migration. The pilot consumer repo can be rolled back independently by reverting its adoption commit.

## Evidence

To be attached after implementation:

- [x] `.claude-plugin/plugin.json` validates (parse OK, all skill paths resolve)
- [x] All 7 SKILL.md files have `name` and `description` frontmatter
- [x] Slim `CLAUDE.md` ≤ 30 lines
- [x] `templates/CONTEXT.md` exists and matches Matt's CONTEXT-FORMAT
- [x] README.md explains the new model
- [x] init.sh trimmed for Tier 2 lazy model
- [ ] Pilot consumer repo: `/resume` works, `/decide` works end-to-end on at least one real decision, `/handoff` works
