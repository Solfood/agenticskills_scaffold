---
name: resume
description: Start-of-session continuity ritual for the Solfood scaffold. Reads project policy, work-index, session-log, proposes the next marker, and surfaces open suite-cross-cutting work. Lazy-creates state files if missing. Use at the start of every session, or any time you need to re-anchor mid-session ("we lost the thread, /resume").
---
...
1. **Read project policy.**
   - If `policies/project-policy.yaml` exists, read it. Note: project name, marker prefix, tracks, suite siblings (if any), risk tolerance, critical assets, mandatory gates.
...
4. **Suite check (if applicable).**
   - If `policies/project-policy.yaml` has a non-empty `suite` block, invoke `/suite-check`. Surface any open SUITE-* markers in sibling repos that could affect today's work.

5. **Propose next marker.**
